...
// 全局强制 IPv4 优先 DNS：避免 Node.js 默认 IPv6 优先导致 gateway.kugou.com 等域名 ETIMEDOUT
try { dns.setDefaultResultOrder('ipv4first'); } catch (e) { /* 旧版 Node.js 不支持 */ }
...
function isKugouCookieDomain(domain) {
  const normalized = String(domain || '').replace(/^\./, '').toLowerCase();
  return normalized === 'kugou.com' || normalized.endsWith('.kugou.com') || normalized.endsWith('.kugou.net');
}
...
async function createWindow() {
  ...
  // 设 NODE_OPTIONS 让子进程（包括 app_win.exe）也使用 IPv4 优先 DNS
...
if (!gotSingleInstanceLock) {
  ...
  if (localServer && localServer.stopKuGouApiServer) localServer.stopKuGouApiServer();
  });
}
